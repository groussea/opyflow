...
for i in range(np.int(N/2)):
    print(np.float(i)/N)
    cap.set(cv2.CAP_PROP_POS_FRAMES, 2*i)
    ret, frame = cap.read()
    framec = frame
    # Good feature + flow calculatition
    frame = opyf.Render.CLAHEbrightness(
        frame, 10, tileGridSize=(32, 32), clipLimit=3)
...
# pour identifier les outliers (le bruit), ça peut etre néfaste car des fois il y a des zones ou t'as bcp de bruit mais que tu les filtres quand même
    ...
    if len(X) > 0:
        # filters
        Dev, Npoints, stD = opyf.Filters.opyfFindPointsWithinRadiusandDeviation(
    ...
    if len(X) > 0:
        ...
        VT = opyf.npInterpolateVTK2D(
            X, V, TargetPoints, ParametreInterpolatorVTK=interp_params)
...
#         'unit':'px'}
        ...
        fig, ax = opyf.Render.opyfPlot(
            grid_x, grid_y, gridVx, gridVy, X, V, setPlot, vis=framec, **render_params)
        fig.set_size_inches(12*1.5, 7*1.5)
        plt.savefig(folder_img_output+'/image_' +
                    format(i, '04.0f')+'.png', format='png', dpi=200)
...
opyf.Render.opyfPlot(grid_x, grid_y, gridVx, gridVy, Xdata,
                     Vdata, setPlot, vis=frame, namefig='Total', **render_params)
Dev, Npoints, stD = opyf.Filters.opyfFindPointsWithinRadiusandDeviation(
    Xdata, (Vdata[:, 0]**2+Vdata[:, 1]**2), 20.)
...
opyf.Render.opyfPlot(grid_x, grid_y, gridVx, gridVy, Xdata1,
                     Vdata1, setPlot, vis=frame, namefig='Total1', **render_params)
Dev, Npoints, stD = opyf.Filters.opyfFindPointsWithinRadiusandDeviation(
    Xdata1, Vdata1[:, 0], 20.)
...
opyf.Render.opyfPlot(grid_x, grid_y, gridVx, gridVy, Xdata2, Vdata2,
                     setPlot, vis=frame, nvec=5000, namefig='Total2', **render_params)
...
if len(Xdata) > 0 and l == listD[select[-1]]:
    ...
    VT = opyf.npInterpolateVTK2D(
        Xdata2, Vdata2, TargetPoints, ParametreInterpolatorVTK=interp_params)
    gridVx = opyf.npTargetPoints2Grid2D([VT[:, 0]], resX, resY)
    gridVy = opyf.npTargetPoints2Grid2D([VT[:, 1]], resX, resY)
...
fig, ax = opyf.Render.opyfPlot(grid_x, grid_y, gridVx, gridVy, Xdata2, Vdata2,
                               setPlot, vis=frame, namefig='Total2', vlim=[0, 50], scale=1000, cmap=cmap)
