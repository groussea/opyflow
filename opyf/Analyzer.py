...
class Analyzer():
    def __init__(self,**args):
        self.scaled=False
        self.ROI=args.get('imageROI',[0,0,self.frameInit.shape[1],self.frameInit.shape[0]])       
...
#        self.Xsamples=[]
    ...
    def set_goodFeaturesToTrackParams(self,**args):
        ...
        self.feature_params = dict( maxCorners = args.get('maxCorners',40000),
                               qualityLevel = args.get('qualityLevel',0.005),
    ...
    def set_filtersParams(self,**args):        
         self.filters_params = dict(RadiusF=args.get('RadiusF',30),
                              ...
                              maxDevinRadius=args.get('maxDevinRadius',np.inf),
    ...
    def set_vlim(self,vlim):
        if self.scaled==False:
            ...
            self.vlimPx=[v/(self.scale*self.fps/self.paramVecTime['step']) for v in vlim]
    ...
    def setGridToInterpolateOn(self,pixLeft=0,pixRight=0,stepHor=2,pixUp=0,pixDown=0,stepVert=2,ROI=None,stepGrid=None):
        ...
        if ROI is not None:
            self.ROImeasure=ROI
            pixLeft, pixRight, pixUp, pixDown = ROI[0], ROI[0]+ROI[2], ROI[1],  ROI[1]+ROI[3]
        else:
    ...
    def setVecTime(self,**args):
            ...
            if self.vec[-1]>self.number_of_frames:
                ...
                print('Consider that [starting_frame+step+(Ntot-1)*shift]  must be smaller than the number of frames' )
                sys.exit()
...
#            self.frameAv=frameav
    ...
    def stepGoodFeaturesToTrackandOpticalFlow(self,pr,i):
        ...
        current_gray=Render.CLAHEbrightness(gray,0,tileGridSize=(20,20),clipLimit=2)
...
# filters ###### important since we have a low quality level for the Good Feature to track and a high Distance Good Flag
            if self.filters_params['minNperRadius']>0:
                Dev,Npoints,stD=Filters.opyfFindPointsWithinRadiusandDeviation(self.X,(self.V[:,0]**2+self.V[:,1]**2),self.filters_params['RadiusF'])
                self.X=Filters.opyfDeletePointCriterion(self.X,Npoints,climmin=self.filters_params['minNperRadius']) 
                self.V=Filters.opyfDeletePointCriterion(self.V,Npoints,climmin=self.filters_params['minNperRadius'])
            if self.filters_params['maxDevinRadius']!=np.inf:    
                Dev,Npoints,stD=Filters.opyfFindPointsWithinRadiusandDeviation(self.X,self.V[:,1],self.filters_params['RadiusF'])
                self.X=Filters.opyfDeletePointCriterion(self.X,Dev,climmax=self.filters_params['maxDevinRadius']) 
                self.V=Filters.opyfDeletePointCriterion(self.V,Dev,climmax=self.filters_params['maxDevinRadius'])                
                Dev,Npoints,stD=Filters.opyfFindPointsWithinRadiusandDeviation(self.X,self.V[:,0],self.filters_params['RadiusF'])
                self.X=Filters.opyfDeletePointCriterion(self.X,Dev,climmax=self.filters_params['maxDevinRadius']) 
                self.V=Filters.opyfDeletePointCriterion(self.V,Dev,climmax=self.filters_params['maxDevinRadius'])            
            ...
            print('')
            self.incr+=1
            print('-------------- [Step '+str(self.incr)+' / '+str(self.paramVecTime['Ntot'])+'] --------------')
            if self.processingMode=='image sequence':
            ...
            print('Displacement max = '+str(np.max(self.V))+' '+self.unit[0]+'/'+self.unit[1])
    ...
    def interpolateOnGrid(self,mode='sequence'):
        ...
        self.Ux=Interpolate.npTargetPoints2Grid2D(self.interpolatedVelocities[:,0],self.Lgrid,self.Hgrid)*self.gridMask
        self.Uy=Interpolate.npTargetPoints2Grid2D(self.interpolatedVelocities[:,1],self.Lgrid,self.Hgrid)*self.gridMask
    ...
    def  extractGoodFeaturesPositionsDisplacementsAndInterpolate(self,display=False,saveImgPath=None,Type='norme',imgFormat='png',**args):
        ...
        for pr,i in zip(self.prev,self.vec):
            ...
            if len(self.X)>0:                
                ...
                self.UxTot.append(np.reshape(self.interpolatedVelocities[:,0],(self.Hgrid,self.Lgrid)))
                self.UyTot.append(np.reshape(self.interpolatedVelocities[:,1],(self.Hgrid,self.Lgrid)))                            
    ...
    def writeGoodFeaturesPositionsAndDisplacements(self,fileFormat='hdf5',outFolder='.',filename=None,fileSequence=False):
        if filename is None:
            filename='good_features_positions_and_opt_flow_from_frame'+ str(self.vec[0]) + 'to' + str(self.vec[-1]) + '_with_step_' +str(self.paramVecTime['step']) + '_and_shift_'+str(self.paramVecTime['shift'])
        if fileFormat=='hdf5':
