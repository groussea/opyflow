...
for i in tqdm.trange(len(prev)):
    ...
    l=listD[select[i]]
    pr=prev[i]
    frame=cv2.imread(folder_src+'/'+l,cv2.IMREAD_ANYDEPTH) #special for tiff images
    if pr==False:
        prev_gray=None
...
#    plt.pause(0.1)
    ...
    frame=np.array(frame,dtype='uint8') #Opencv only deal with uint8 images
...
#The function CLAHE help to enhance and obtain good balance between the two images
    frame=opyf.Render.CLAHEbrightness(frame,0,tileGridSize=(20,20),clipLimit=2)
##    plt.imshow(frame)
...
#        For the momentwe didnt interpolate data on grid
        ...
        plt.savefig(folder_img_output+'/Step'+format(incr,'04.0f')+'.png',format='png',dpi=100)       
...
#%% Filter ouliers
Devx,Npointsx,stDx=opyf.Filters.opyfFindPointsWithinRadiusandDeviation(Xdata[:,:],Vdata[:,0],5)
Devy,Npointsy,stDy=opyf.Filters.opyfFindPointsWithinRadiusandDeviation(Xdata[:,:],Vdata[:,1],5)
#%%
...
plt.figure()
nx, binsx, patches = plt.hist(stDx, 2000, normed=1, facecolor='red', alpha=0.75,label='DX')
...
XT=opyf.Interpolate.npGrid2TargetPoint2D(grid_x,grid_y) #VTK only deal with [X Y] format
...
interp_params= dict (Radius=15., # it is not necessary to perform unterpolation on a high radius since we have a high number of values
                     ...
                     kernel='Gaussian',
                     scaleInterp=1.)
VT_mean=opyf.npInterpolateVTK2D(Xdata,Vdata,XT,ParametreInterpolatorVTK=interp_params) # may take a while if X large
...
colors = [(11./255,22./255,33./255,1.),(33./255,66./255,99./255,1.),(33./255,66./255,99./255,0.), (204./255.,204./255,0,1),(0.6,0,0,1)]
...
fig,ax=opyf.Render.opyfPlot(grid_x, grid_y,gridVx_mean,gridVy_mean,Xdata,Vdata,setPlot,vis=frame,namefig='F',cmap=cmap,normalize=True,width=0.0015,**render_params)
...
Dev,Npoints,stD=opyf.opyfFindPointsWithinRadiusandDeviation(XT,VT_full[:,1],15)
X1=opyf.opyfDeletePointCriterion(XT,Dev,climmax=1.) 
V1=opyf.opyfDeletePointCriterion(VT_full,Dev,climmax=1.)
VT_full2=opyf.npInterpolateVTK2D(X1,V1,XT,ParametreInterpolatorVTK=interp_params)

gridVx_mean2=opyf.Interpolate.npTargetPoints2Grid2D(VT_full2[:,0],resX,resY)
gridVy_mean2=opyf.Interpolate.npTargetPoints2Grid2D(VT_full2[:,1],resX,resY)
opyf.Render.opyfPlot(grid_x, grid_y,gridVx_mean,gridVy_mean,XT,VT_full,setPlot,vis=frame,namefig='F',cmap=cmap,**render_params)
...
Devx,Npoints,stDx=opyf.Filters.opyfFindPointsWithinRadiusandDeviation(Xdata[1:2000000,:],Vdata[1:2000000,0],5.)
Devy,Npoints,stDy=opyf.Filters.opyfFindPointsWithinRadiusandDeviation(Xdata[1:2000000,:],Vdata[1:2000000,1],5.)
...
interp_params= dict (Radius=5., # it is not necessary to perform unterpolation on a high radius since we have a high number of values
                     ...
                     kernel='Gaussian',
                     scaleInterp=1.)
stD_full=opyf.npInterpolateVTK2D(Xdata2, np.transpose([stDx2,stDx2]),XT,ParametreInterpolatorVTK=interp_params)
...
colors = [(11./255,22./255,33./255,0.),(33./255,66./255,99./255,1.), (204./255.,204./255,0,1),(0.6,0,0,1)]
...
render_params= dict(Ptype='horizontal',
                    vlim=[0.,10.],
                    scale=1000)
opyf.Render.opyfPlot(grid_x, grid_y,gridVx_Dev,gridVy_Dev,XT,VT_full,setPlot,vis=frame,namefig='F',cmap=cmap,**render_params)
...
opyf.Files.tecplot_WriteRectilinearMesh(folder_outputs+'/mean_vec.tec', Xvec, Yvec, variables)
...
out=opyf.tecplot_reader('/media/gauthier/Data_1/TAF/OPyF-Project/Opyf-PIV-Challenge-2014-Test/CaseA/output_with_mask/mean_vec.tec')
VT_mean=out[:,2:4]
...
#    frame=cv2.imread(folder_src+'/'+l)
    ...
    if len(X)>0:
        ...
        VT=opyf.npInterpolateVTK2D(X,V,XT,ParametreInterpolatorVTK=interp_params)
        ...
        render_params= dict(Ptype='norme',
                    vlim=[0.,20.],
                    scale=1000)
        gridVx_rms=opyf.Interpolate.npTargetPoints2Grid2D(VT_rmstemp[:,0],resX,resY)
        gridVy_rms=opyf.Interpolate.npTargetPoints2Grid2D(VT_rmstemp[:,1],resX,resY)
        setPlot={'DisplayVis':True,
                         'DisplayField':True,
                 'QuiverOnFieldColored':False,
                 'QuiverOnField':False,
        ...
        fig,ax=opyf.Render.opyfPlot(grid_x_PIVC,grid_y_PIVC,gridVx_rms,gridVy_rms,XT,VT_rms,setPlot,vis=frame,namefig='QUiverF',**render_params)         
        
        opyf.Render.opyfText(fulldict,ax=ax,pos=(1200,1000))
        plt.pause(0.1)
        plt.savefig(folder_img_output3+'/Step'+format(incr,'04.0f')+'.png',format='png',dpi=100)              
...
variables=(("rms_Vx", VT_rms[:,0]),("rms_Vy",VT_rms[:,1]),("flag",flag))
    
opyf.Files.tecplot_WriteRectilinearMesh(folder_outputs+'/rms_vec.tec', Xvec, Yvec, variables)
...
fig,ax=opyf.Render.opyfPlot(grid_x_PIVC,grid_y_PIVC,gridVx_rms,gridVy_rms,XT,VT_rms,setPlot,vis=frame,cmap=cmap,namefig='QUiverF',**render_params)         
...
nx, binsx, patches = plt.hist(histDX[0], 2000, normed=1, facecolor='red', alpha=0.75,label='DX')
ny, binsy, patches = plt.hist(histDY[0], 2000, normed=1, facecolor='green', alpha=0.75,label='DY')
plt.legend()
...
plt.plot(binsx[0:-1]+(binsx[1]-binsx[0]),nx,label='DX',alpha=0.75,linewidth=2.)
plt.plot(binsy[0:-1]+(binsy[1]-binsy[0]),ny,label='DY',alpha=0.75,linewidth=2.)
plt.xlabel('DX,DY(in px)')
plt.ylabel('pdf')
plt.text(25,0.25,'mean DX=' +format(np.mean(histDX),'.2f'))
plt.text(25,0.22,'mean DY='+ format(np.mean(histDY),'.2f'))
